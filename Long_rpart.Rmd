---
title: "Basic CLassification Tree"
author: "WATO"
date: "10/20/2020"
output:
  html_document:
...
plot(rpartImp, top = 25, scales = list(y = list(cex = .95)), main = "Variable Importance in rpart Model (Original Dataset)")

# Plot of Model Tuning
plot(rpartFit, ylab = "ROC AUC (Hold-Out Data)")

# Predict testX_dmy
rpart_evalResults <- predict(rpartFit,newdata = testX_dmy,type = "prob") # probability for class Y and N
rpart_evalResults$Prediction <- predict(rpartFit,newdata = testX_dmy) # Prediction result
rpart_evalResults$True <- testY # True class
rpart_evalResults

# Validation on test set
confusionMatrix(rpart_evalResults$Prediction, rpart_evalResults$True, positive = "Y")

# Plot ROC of test set
library(pROC)
rpartRoc_test <- roc(response = rpart_evalResults$True,
                predictor = rpart_evalResults$Y,
                levels =c("N","Y"),
                direction = "<")
plot(rpartRoc_test, type = "s", legacy.axes = TRUE, main = "ROC of rpart on test set")
...
library(pROC)
rpartRoc_training <- roc(response = rpartFit$pred$obs,
                  predictor = rpartFit$pred$Y,
                  levels =c("N","Y"),
                  direction = "<")

# Get the new cutoff
rpartThreshN <- coords(rpartRoc_training, x = "best", best.method = "closest.topleft") 
rpartThreshN

# Predict testX_dmy with new cutoff
rpart_evalResults$Prediction_NewCut <- factor(ifelse(rpart_evalResults$Y > rpartThreshN$threshold, 
                                                "Y", "N"),
                                         levels = c("Y","N")) # Using new cutoff to predict test set
rpart_evalResults

# Validation on test set
confusionMatrix(rpart_evalResults$Prediction_NewCut, rpart_evalResults$True, positive = "Y")
...
plot(rpartRoc_training, type = "s", legacy.axes = TRUE, 
     main = "ROC of rpart model on training set")
points(rpartThreshN$specificity,
       rpartThreshN$sensitivity,pch=19,col="red")
points(rpartThreshO$specificity,
       rpartThreshO$sensitivity,pch=19,col="blue")

# Plot ROC for test set
#### Get the specificity and sensitivity for the original threshold of 0.5 on test set
rpartThreshO_test <- coords(rpartRoc_test, x = 0.5, input = "threshold", transpose = FALSE) 
rpartThreshO_test
#### Get The specificity and sensitivity for the alternate threshold of 0.027 on test set
rpartThreshN_test <- coords(rpartRoc_test, x = rpartThreshN$threshold, input = "threshold", transpose = FALSE) 
rpartThreshN_test
#### Visualize the original threshold and new threshold on test set
plot(rpartRoc_test, type = "s", legacy.axes = TRUE, 
     main = "ROC of rpart model on test set")
points(rpartThreshN_test$specificity,
       rpartThreshN_test$sensitivity,pch=19,col="red")
points(rpartThreshO_test$specificity,
       rpartThreshO_test$sensitivity,pch=19,col="blue")
...
library(sparseLDA)
set.seed(100)
rpartFit_up <- train(x = upSampledTrainX_dmy, 
                y = upSampledTrainY,
                method = "rpart",
                tuneLength = 30,
                metric = "ROC",
...
# Variable importance
rpartImp_up <- varImp(rpartFit_up, scale = FALSE)
plot(rpartImp_up, top = 25, scales = list(y = list(cex = .95)), main = "rpart (UpSampled Dataset)")

# Plot of Model Tuning
plot(rpartFit_up, ylab = "ROC AUC (Hold-Out Data)")

# Predict testX_dmy
rpart_up_evalResults <- predict(rpartFit_up,newdata = testX_dmy,type = "prob") # probability for class Y and N
rpart_up_evalResults$Prediction <- predict(rpartFit_up,newdata = testX_dmy) # Prediction result
rpart_up_evalResults$True <- testY # True class
rpart_up_evalResults

# Validation on test set
confusionMatrix(rpart_up_evalResults$Prediction, rpart_up_evalResults$True, positive = "Y")

# Plot ROC of test set
library(pROC)
rpartRoc_up_test <- roc(response = rpart_up_evalResults$True,
                predictor = rpart_up_evalResults$Y,
                levels =c("N","Y"),
                direction = "<")
plot(rpartRoc_up_test, type = "s", legacy.axes = TRUE, main = "ROC of rpart on test set")

# Get AUC value of test set
rpartRoc_up_test$auc
...
# Create the list for Plotting ROC of the training set
library(pROC)
rpartRoc_up_training <- roc(response = rpartFit_up$pred$obs,
                  predictor = rpartFit_up$pred$Y,
                  levels =c("N","Y"),
                  direction = "<")

# Get the new cutoff
rpartThreshN_up <- coords(rpartRoc_up_training, x = "best", best.method = "closest.topleft") 
rpartThreshN_up

# Predict testX_dmy with new cutoff
rpart_up_evalResults$Prediction_NewCut <- factor(ifelse(rpart_up_evalResults$Y > rpartThreshN_up$threshold, "Y", "N"),
                                         levels = c("Y","N")) # Using new cutoff to predict test set
rpart_up_evalResults

# Validation on test set
confusionMatrix(rpart_up_evalResults$Prediction_NewCut, rpart_up_evalResults$True, positive = "Y")
...
# Get AUC value of training set
print("training set")
rpartRoc_up_training$auc

# Plot ROC for training set
#### Get the specificity and sensitivity for the original threshold of 0.5 on training set
rpartThreshO_up <- coords(rpartRoc_up_training, x = 0.5, input = "threshold", transpose = FALSE) 
rpartThreshO_up
#### Get The specificity and sensitivity for the alternate threshold of 0.2398 on training set
rpartThreshN_up
#### Visualize the original threshold and new threshold on training set
plot(rpartRoc_up_training, type = "s", legacy.axes = TRUE, 
     main = "ROC of rpart model on training set")
points(rpartThreshN_up$specificity,
       rpartThreshN_up$sensitivity,pch=19,col="red")
points(rpartThreshO_up$specificity,
       rpartThreshO_up$sensitivity,pch=19,col="blue")

# Plot ROC for test set
#### Get the specificity and sensitivity for the original threshold of 0.5 on test set
rpartThreshO_up_test <- coords(rpartRoc_up_test, x = 0.5, input = "threshold", transpose = FALSE) 
rpartThreshO_up_test
#### Get The specificity and sensitivity for the alternate threshold of 0.2398 on test set
rpartThreshN_up_test <- coords(rpartRoc_up_test, x = rpartThreshN_up$threshold, input = "threshold", transpose = FALSE) 
rpartThreshN_up_test
#### Visualize the original threshold and new threshold on test set
plot(rpartRoc_up_test, type = "s", legacy.axes = TRUE, 
     main = "ROC of rpart model on test set")
points(rpartThreshN_up_test$specificity,
       rpartThreshN_up_test$sensitivity,pch=19,col="red")
points(rpartThreshO_up_test$specificity,
       rpartThreshO_up_test$sensitivity,pch=19,col="blue")
...
```{r}
plot(rpartFit, scales = list(x = list(log = 10)), ylab = "ROC AUC (Hold-Out Data)")
plot(rpartFit_up, scales = list(x = list(log = 10)), add = TRUE, ylab = "ROC AUC (Hold-Out Data)")
...
plot(rpartRoc_training, type = "s", legacy.axes = TRUE, 
     main = "ROC of rpart model on training set")
points(rpartThreshN$specificity,
       rpartThreshN$sensitivity,pch=17,col="red")
points(rpartThreshO$specificity,
       rpartThreshO$sensitivity,pch=17,col="blue")
# Upsampled data
plot(rpartRoc_up_training, type = "s", legacy.axes = TRUE, add = TRUE,col = rgb(.5, .5, .5, .5),
     main = "ROC of rpart model on training set")
points(rpartThreshN_up$specificity,
       rpartThreshN_up$sensitivity,pch=19,col="red")
points(rpartThreshO_up$specificity,
...
plot(rpartRoc_test, type = "s", legacy.axes = TRUE, 
     main = "ROC of rpart model on test set")
points(rpartThreshN_test$specificity,
       rpartThreshN_test$sensitivity,pch=17,col="red")
points(rpartThreshO_test$specificity,
       rpartThreshO_test$sensitivity,pch=17,col="blue")
# Up-sampling dataset
plot(rpartRoc_up_test, type = "s", legacy.axes = TRUE, add = TRUE,col = rgb(.5, .5, .5, .5),
     main = "ROC of rpart model on test set")
points(rpartThreshN_up_test$specificity,
       rpartThreshN_up_test$sensitivity,pch=19,col="red")
points(rpartThreshO_up_test$specificity,
