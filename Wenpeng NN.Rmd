...
```{r}
# Fit model
nnetGrid <- expand.grid(size = c(1,5,10), decay = c(0, 2)) # size: number of hidden unites
maxSize <- max(nnetGrid$size)
library(nnet)
set.seed(100)
NNFit <- train(x = trainX_dmy, 
                  y = trainY,
                  method = "nnet",
                  tuneGrid = nnetGrid,
                  metric = "ROC",
                  MaxNWts = 1*(maxSize * (length(trainX_dmy) + 1) + maxSize + 1),
                  trControl = ctrl)
NNFit

# Variable importance
NNImp <- varImp(NNFit, scale = FALSE)
plot(NNImp, top = 25, scales = list(y = list(cex = .95)), main = "Neural Network")

# Plot of Model Tuning
plot(NNFit,  ylab = "ROC AUC (Hold-Out Data)")

# Predict testX_dmy
NN_evalResults <- predict(NNFit,newdata = testX_dmy,type = "prob") # probability for class Y and N
NN_evalResults$Prediction <- predict(NNFit,newdata = testX_dmy) # Prediction result
NN_evalResults$True <- testY # True class
NN_evalResults

# Validation on test set
confusionMatrix(NN_evalResults$Prediction, NN_evalResults$True, positive = "Y")

# Plot ROC of test set
library(pROC)
NNRoc_test <- roc(response = NN_evalResults$True,
                     predictor = NN_evalResults$Y,
                     levels =c("N","Y"),
                     direction = "<")
plot(NNRoc_test, type = "s", legacy.axes = TRUE, main = "ROC of Neural Networks on test set")
...
library(pROC)
NNRoc_training <- roc(response = NNFit$pred$obs,
                  predictor = NNFit$pred$Y,
                  levels =c("N","Y"),
                  direction = "<")

# Get the new cutoff
NNThreshN <- coords(NNRoc_training, x = "best", best.method = "closest.topleft") 
NNThreshN

# Predict testX_dmy with new cutoff
NN_evalResults$Prediction_NewCut <- factor(ifelse(NN_evalResults$Y > NNThreshN$threshold, 
                                                "Y", "N"),
                                         levels = c("Y","N")) # Using new cutoff to predict test set
NN_evalResults

# Validation on test set
confusionMatrix(NN_evalResults$Prediction_NewCut, NN_evalResults$True, positive = "Y")
...
NNThreshO <- coords(NNRoc_training, x = 0.5, input = "threshold", transpose = FALSE) 
NNThreshO
...
plot(NNRoc_training, type = "s", legacy.axes = TRUE, 
     main = "ROC of Neural Network model on training set")
points(NNThreshN$specificity,
       NNThreshN$sensitivity,pch=19,col="red")
points(NNThreshO$specificity,
       NNThreshO$sensitivity,pch=19,col="blue")

# Plot ROC for test set
#### Get the specificity and sensitivity for the original threshold of 0.5 on test set
NNThreshO_test <- coords(NNRoc_test, x = 0.5, input = "threshold", transpose = FALSE) 
NNThreshO_test
#### Get The specificity and sensitivity for the alternate threshold of 0.027 on test set
NNThreshN_test <- coords(NNRoc_test, x = NNThreshN$threshold, input = "threshold", transpose = FALSE) 
NNThreshN_test
#### Visualize the original threshold and new threshold on test set
plot(NNRoc_test, type = "s", legacy.axes = TRUE, 
     main = "ROC of Neural Network model on test set")
points(NNThreshN_test$specificity,
       NNThreshN_test$sensitivity,pch=19,col="red")
points(NNThreshO_test$specificity,
       NNThreshO_test$sensitivity,pch=19,col="blue")
...
maxSize <- max(nnetGrid$size)
set.seed(100)
NNFit_up <- train(x = upSampledTrainX_dmy, 
                  y = upSampledTrainY,
                  method = "nnet",
                  tuneGrid = nnetGrid,
                  metric = "ROC",
                  MaxNWts = 1*(maxSize * (length(upSampledTrainX_dmy) + 1) + maxSize + 1),
                  trControl = ctrl)
NNFit_up


# Variable importance
NNImp_up <- varImp(NNFit_up, scale = FALSE)
plot(NNImp_up, top = 25, scales = list(y = list(cex = .95)), main = "Neural Networks (Up-Sampling Dataset)")

# Plot of Model Tuning
plot(NNFit_up,  ylab = "ROC AUC (Hold-Out Data)")

# Predict testX_dmy
NN_up_evalResults <- predict(NNFit_up,newdata = testX_dmy,type = "prob") # probability for class Y and N
NN_up_evalResults$Prediction <- predict(NNFit_up,newdata = testX_dmy) # Prediction result
NN_up_evalResults$True <- testY # True class
NN_up_evalResults

# Validation on test set
confusionMatrix(NN_up_evalResults$Prediction, NN_up_evalResults$True, positive = "Y")

# Plot ROC of test set
library(pROC)
NNRoc_up_test <- roc(response = NN_up_evalResults$True,
                predictor = NN_up_evalResults$Y,
                levels =c("N","Y"),
                direction = "<")
plot(NNRoc_up_test, type = "s", legacy.axes = TRUE, main = "ROC of Neural Networks on test set")
...
# Create the list for Plotting ROC of the training set
library(pROC)
NNRoc_up_training <- roc(response = NNFit_up$pred$obs,
                  predictor = NNFit_up$pred$Y,
                  levels =c("N","Y"),
                  direction = "<")

# Get the new cutoff
NNThreshN_up <- coords(NNRoc_up_training, x = "best", best.method = "closest.topleft") 
NNThreshN_up

# Predict testX_dmy with new cutoff
NN_up_evalResults$Prediction_NewCut <- factor(ifelse(NN_up_evalResults$Y > NNThreshN_up$threshold, "Y", "N"),
                                         levels = c("Y","N")) # Using new cutoff to predict test set
NN_up_evalResults

# Validation on test set
confusionMatrix(NN_up_evalResults$Prediction_NewCut, NN_up_evalResults$True, positive = "Y")
...
# Get AUC value of training set
print("training set")
NNRoc_up_training$auc

# Plot ROC for training set
#### Get the specificity and sensitivity for the original threshold of 0.5 on training set
NNThreshO_up <- coords(NNRoc_up_training, x = 0.5, input = "threshold", transpose = FALSE) 
NNThreshO_up
#### Get The specificity and sensitivity for the alternate threshold of 0.2398 on training set
NNThreshN_up
#### Visualize the original threshold and new threshold on training set
plot(NNRoc_up_training, type = "s", legacy.axes = TRUE, 
     main = "ROC of Neural Networks model on training set")
points(NNThreshN_up$specificity,
       NNThreshN_up$sensitivity,pch=19,col="red")
points(NNThreshO_up$specificity,
       NNThreshO_up$sensitivity,pch=19,col="blue")

# Plot ROC for test set
#### Get the specificity and sensitivity for the original threshold of 0.5 on test set
NNThreshO_up_test <- coords(NNRoc_up_test, x = 0.5, input = "threshold", transpose = FALSE) 
NNThreshO_up_test
#### Get The specificity and sensitivity for the alternate threshold of 0.2398 on test set
NNThreshN_up_test <- coords(NNRoc_up_test, x = NNThreshN_up$threshold, input = "threshold", transpose = FALSE) 
NNThreshN_up_test
#### Visualize the original threshold and new threshold on test set
plot(NNRoc_up_test, type = "s", legacy.axes = TRUE, 
     main = "ROC of Neural Networks model on test set")
points(NNThreshN_up_test$specificity,
       NNThreshN_up_test$sensitivity,pch=19,col="red")
points(NNThreshO_up_test$specificity,
       NNThreshO_up_test$sensitivity,pch=19,col="blue")
...
```{r}
plot(NNFit, ylab = "ROC AUC (Hold-Out Data)")
plot(NNFit_up,  add = TRUE, ylab = "ROC AUC (Hold-Out Data)")
...
plot(NNRoc_training, type = "s", legacy.axes = TRUE, 
     main = "ROC of Neural Networks model on training set")
points(NNThreshN$specificity,
       NNThreshN$sensitivity,pch=17,col="red")
points(NNThreshO$specificity,
       NNThreshO$sensitivity,pch=17,col="blue")
# Upsampled data
plot(NNRoc_up_training, type = "s", legacy.axes = TRUE, add = TRUE,col = rgb(.5, .5, .5, .5),
     main = "ROC of Neural Networks model on training set")
points(NNThreshN_up$specificity,
       NNThreshN_up$sensitivity,pch=19,col="red")
points(NNThreshO_up$specificity,
...
plot(NNRoc_test, type = "s", legacy.axes = TRUE, 
     main = "ROC of Neural Networks model on test set")
points(NNThreshN_test$specificity,
       NNThreshN_test$sensitivity,pch=17,col="red")
points(NNThreshO_test$specificity,
       NNThreshO_test$sensitivity,pch=17,col="blue")
# Up-sampling dataset
plot(NNRoc_up_test, type = "s", legacy.axes = TRUE, add = TRUE,col = rgb(.5, .5, .5, .5),
     main = "ROC of Neural Networks model on test set")
points(NNThreshN_up_test$specificity,
       NNThreshN_up_test$sensitivity,pch=19,col="red")
points(NNThreshO_up_test$specificity,
