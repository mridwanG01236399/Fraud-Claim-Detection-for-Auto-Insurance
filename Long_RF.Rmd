...
mtryValues <- c(30, 40, 50, 60, 70, 100)
set.seed(100)
rfFit <- train(x = trainX_dmy, 
                y = trainY,
                method = "rf",
               ntree = 500,
...
plot(rfImp, top = 10, scales = list(y = list(cex = .95)), main = "Top 10 Important Variables for rf Model (Original Dataset)")

# Plot of Model Tuning
plot(rfFit, ylab = "ROC AUC (Hold-Out Data)")

# Predict testX_dmy
rf_evalResults <- predict(rfFit,newdata = testX_dmy,type = "prob") # probability for class Y and N
rf_evalResults$Prediction <- predict(rfFit,newdata = testX_dmy) # Prediction result
rf_evalResults$True <- testY # True class
rf_evalResults

# Validation on test set
confusionMatrix(rf_evalResults$Prediction, rf_evalResults$True, positive = "Y")

# Plot ROC of test set
library(pROC)
rfRoc_test <- roc(response = rf_evalResults$True,
                predictor = rf_evalResults$Y,
                levels =c("N","Y"),
                direction = "<")
plot(rfRoc_test, type = "s", legacy.axes = TRUE, main = "ROC of rf on test set")
...
library(pROC)
rfRoc_training <- roc(response = rfFit$pred$obs,
                  predictor = rfFit$pred$Y,
                  levels =c("N","Y"),
                  direction = "<")

# Get the new cutoff
rfThreshN <- coords(rfRoc_training, x = "best", best.method = "closest.topleft") 
rfThreshN

# Predict testX_dmy with new cutoff
rf_evalResults$Prediction_NewCut <- factor(ifelse(rf_evalResults$Y > rfThreshN$threshold, 
                                                "Y", "N"),
                                         levels = c("Y","N")) # Using new cutoff to predict test set
rf_evalResults

# Validation on test set
confusionMatrix(rf_evalResults$Prediction_NewCut, rf_evalResults$True, positive = "Y")
...
rfThreshO <- coords(rfRoc_training, x = 0.5, input = "threshold", transpose = FALSE) 
rfThreshO
...
plot(rfRoc_training, type = "s", legacy.axes = TRUE, 
     main = "ROC of rf model on training set")
points(rfThreshN$specificity,
       rfThreshN$sensitivity,pch=19,col="red")
points(rfThreshO$specificity,
       rfThreshO$sensitivity,pch=19,col="blue")

# Plot ROC for test set
#### Get the specificity and sensitivity for the original threshold of 0.5 on test set
rfThreshO_test <- coords(rfRoc_test, x = 0.5, input = "threshold", transpose = FALSE) 
rfThreshO_test
#### Get The specificity and sensitivity for the alternate threshold of 0.027 on test set
rfThreshN_test <- coords(rfRoc_test, x = rfThreshN$threshold, input = "threshold", transpose = FALSE) 
rfThreshN_test
#### Visualize the original threshold and new threshold on test set
plot(rfRoc_test, type = "s", legacy.axes = TRUE, 
     main = "ROC of rf model on test set")
points(rfThreshN_test$specificity,
       rfThreshN_test$sensitivity,pch=19,col="red")
points(rfThreshO_test$specificity,
       rfThreshO_test$sensitivity,pch=19,col="blue")
...
```{r message=FALSE, warning=FALSE}
# Fit model
mtryValues <- c(10, 20, 30, 40, 50, 60)
set.seed(100)
rfFit_up <- train(x = upSampledTrainX_dmy, 
                y = upSampledTrainY,
                method = "rf",
                ntree = 500,
                tuneGrid = data.frame(mtry = mtryValues),
...
# Variable importance
rfImp_up <- varImp(rfFit_up, scale = FALSE)
plot(rfImp_up, top = 25, scales = list(y = list(cex = .95)), main = "Variable Importance in rf Model (Up-Sampling Dataset)")

# Plot of Model Tuning
plot(rfFit_up, scales = list(x = list(log = 10)), ylab = "ROC AUC (Hold-Out Data)")

# Predict testX_dmy
rf_up_evalResults <- predict(rfFit_up,newdata = testX_dmy,type = "prob") # probability for class Y and N
rf_up_evalResults$Prediction <- predict(rfFit_up,newdata = testX_dmy) # Prediction result
rf_up_evalResults$True <- testY # True class
rf_up_evalResults

# Validation on test set
confusionMatrix(rf_up_evalResults$Prediction, rf_up_evalResults$True, positive = "Y")

# Plot ROC of test set
library(pROC)
rfRoc_up_test <- roc(response = rf_up_evalResults$True,
                predictor = rf_up_evalResults$Y,
                levels =c("N","Y"),
                direction = "<")
plot(rfRoc_up_test, type = "s", legacy.axes = TRUE, main = "ROC of rf on test set")
...
# Create the list for Plotting ROC of the training set
library(pROC)
rfRoc_up_training <- roc(response = rfFit_up$pred$obs,
                  predictor = rfFit_up$pred$Y,
                  levels =c("N","Y"),
                  direction = "<")

# Get the new cutoff
rfThreshN_up <- coords(rfRoc_up_training, x = "best", best.method = "closest.topleft") 
rfThreshN_up

# Predict testX_dmy with new cutoff
rf_up_evalResults$Prediction_NewCut <- factor(ifelse(rf_up_evalResults$Y > rfThreshN_up$threshold, "Y", "N"),
                                         levels = c("Y","N")) # Using new cutoff to predict test set
rf_up_evalResults

# Validation on test set
confusionMatrix(rf_up_evalResults$Prediction_NewCut, rf_up_evalResults$True, positive = "Y")
...
# Get AUC value of training set
print("training set")
rfRoc_up_training$auc

# Plot ROC for training set
#### Get the specificity and sensitivity for the original threshold of 0.5 on training set
rfThreshO_up <- coords(rfRoc_up_training, x = 0.5, input = "threshold", transpose = FALSE) 
rfThreshO_up
#### Get The specificity and sensitivity for the alternate threshold of 0.2398 on training set
rfThreshN_up
#### Visualize the original threshold and new threshold on training set
plot(rfRoc_up_training, type = "s", legacy.axes = TRUE, 
     main = "ROC of rf model on training set")
points(rfThreshN_up$specificity,
       rfThreshN_up$sensitivity,pch=19,col="red")
points(rfThreshO_up$specificity,
       rfThreshO_up$sensitivity,pch=19,col="blue")

# Plot ROC for test set
#### Get the specificity and sensitivity for the original threshold of 0.5 on test set
rfThreshO_up_test <- coords(rfRoc_up_test, x = 0.5, input = "threshold", transpose = FALSE) 
rfThreshO_up_test
#### Get The specificity and sensitivity for the alternate threshold of 0.2398 on test set
rfThreshN_up_test <- coords(rfRoc_up_test, x = rfThreshN_up$threshold, input = "threshold", transpose = FALSE) 
rfThreshN_up_test
#### Visualize the original threshold and new threshold on test set
plot(rfRoc_up_test, type = "s", legacy.axes = TRUE, 
     main = "ROC of rf model on test set")
points(rfThreshN_up_test$specificity,
       rfThreshN_up_test$sensitivity,pch=19,col="red")
points(rfThreshO_up_test$specificity,
       rfThreshO_up_test$sensitivity,pch=19,col="blue")
...
```{r}
plot(rfFit, ylab = "ROC AUC (Hold-Out Data)")
plot(rfFit_up, add = TRUE, ylab = "ROC AUC (Hold-Out Data)")
...
plot(rfRoc_training, type = "s", legacy.axes = TRUE, 
     main = "ROC of rf model on training set")
points(rfThreshN$specificity,
       rfThreshN$sensitivity,pch=17,col="red")
points(rfThreshO$specificity,
       rfThreshO$sensitivity,pch=17,col="blue")
# Upsampled data
plot(rfRoc_up_training, type = "s", legacy.axes = TRUE, add = TRUE,col = rgb(.5, .5, .5, .5),
     main = "ROC of rf model on training set")
points(rfThreshN_up$specificity,
       rfThreshN_up$sensitivity,pch=19,col="red")
points(rfThreshO_up$specificity,
...
plot(rfRoc_test, type = "s", legacy.axes = TRUE, 
     main = "ROC of rf model on test set")
points(rfThreshN_test$specificity,
       rfThreshN_test$sensitivity,pch=17,col="red")
points(rfThreshO_test$specificity,
       rfThreshO_test$sensitivity,pch=17,col="blue")
# Up-sampling dataset
plot(rfRoc_up_test, type = "s", legacy.axes = TRUE, add = TRUE,col = rgb(.5, .5, .5, .5),
     main = "ROC of rf model on test set")
points(rfThreshN_up_test$specificity,
       rfThreshN_up_test$sensitivity,pch=19,col="red")
points(rfThreshO_up_test$specificity,
